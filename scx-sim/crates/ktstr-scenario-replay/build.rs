...
//!
//! The same job as `embed_harness`'s: re-emit the non-transitive kfunc-export
//! link args and build `libscx_simple.so` the way an external embedder
//! (cargo-ktstr) would -- through scx_simulator's `links = "scxsim"` metadata
