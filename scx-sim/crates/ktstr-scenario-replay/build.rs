...
//!
//! Byte-for-byte the same job as `embed_harness`'s: re-emit the non-transitive
//! kfunc-export link args and build `libscx_simple.so` the way an external
//! embedder (cargo-ktstr) would. Duplicated rather than shared because it IS
//! the contract an embedder must satisfy independently — see embed_harness.
//!
