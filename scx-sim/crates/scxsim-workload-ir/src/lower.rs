...
/// Frozen second-generation *shape* identity.
///
/// Deliberately SEPARATE from [`IO_MODEL_V1_PROFILE_ID`] and its manifest
/// digest, which continue to bind the two time TOTALS to v1's evidence. v2
/// does not refit the totals — it apportions them — so each calibrated
/// quantity stays bound to the evidence that actually produced it. A caller
/// that supplies v1's totals is still asserting v1's calibration conditions;
/// the shape digest asserts the additional ones v2 measured.
pub const IO_MODEL_V2_SHAPE_ID: &str = "io-sync-write-raw-vda-v2-shape";

/// Per-cycle on-CPU slice shape: the 33 inverse-CDF points at `(i+0.5)/33` of
/// the calibrated marginal slice distribution, in nanoseconds as measured.
///
/// These are WEIGHTS, not durations. [`apportion_exact`] rescales them so the
/// emitted slices sum to v1's frozen total exactly, which is what makes
/// dimension (1) a tie by construction rather than by approximation. Storing
/// them at measured scale rather than pre-normalised keeps the provenance
/// legible and lets the rescaling factor itself be reported.
///
/// The measured scale is NOT claimed to be correct in absolute terms: summed
/// over a run, wprof's per-slice durations fall short of the kernel's own
/// `sum_exec_runtime` by a VARIABLE factor, measured at 2.23x to 4.59x
/// (median 3.21x) across the 15 eligible Set B runs at 6-56 MiB. It is not a
/// stable multiplier — it spans 2x across runs — so no single correction
/// factor exists, which is itself the reason the scale cannot be used.
///
/// Why the kernel is taken as the reference and wprof as the outlier: over
/// those same 15 runs, wprof's ONCPU slice count minus `schedstat` `pcount` is
/// exactly +1 in 15 of 15. Both instruments therefore see the SAME on-CPU
/// episodes, and `sum_exec_runtime` is the kernel's own integration of exactly
/// those episodes' durations. The disagreement is localised to the durations,
/// and the kernel measures them more directly.
///
/// Do NOT restate this as "two independent kernel instruments corroborate each
/// other". `ru_stime`, `CLOCK_THREAD_CPUTIME_ID` and `schedstat` field 0 are
/// three readings of ONE quantity, `se.sum_exec_runtime` — `cputime_adjust()`
/// pins `ru_stime + ru_utime` to it, so the tick only splits user from system
/// and their agreement is an identity rather than evidence. See
/// `experiments/io_model_v2_20260814/reports/GATE1_EVIDENCE_REVIEW_20260814.md`
/// finding F2; reproduce with `reports/gate1_recheck.py --instruments`.
///
/// Only the SHAPE is used, and the rescaling discards the disputed scale.
/// This is why the calibration is honest despite the open discrepancy.
/// Generated by `experiments/io_model_v2_20260814/tools/fit_shape.py`; do not
/// hand-edit. The jump between points 16 and 17 is the measured bimodality —
/// 16 short alternations and 16 long ones per cycle, plus the `fdatasync`.
const IO_MODEL_V2_ONCPU_SHAPE_NS: [u64; 33] = [
    288903, 296217, 304396, 317224, 330393, 341290, 344446, 347305, 351164, 353313, 363546, 368608,
    374672, 394824, 405352, 462271, 565333, 1285282, 1315146, 1331829, 1339746, 1346048, 1354578,
    1362495, 1374468, 1395247, 1434183, 1466264, 1520961, 1613039, 1670436, 1828179, 2393924,
];
/// Per-cycle non-running interval shape, on the same 33-point convention.
const IO_MODEL_V2_OFFCPU_SHAPE_NS: [u64; 33] = [
    357531, 377885, 385136, 399017, 443534, 454689, 473713, 479941, 485135, 489896, 495343, 501111,
    508281, 519396, 535038, 562588, 674793, 1029332, 1079920, 1103992, 1127092, 1161297, 1202110,
    1241107, 1268185, 1290366, 1319281, 1355491, 1395616, 1471598, 1597419, 1945095, 2587551,
];

/// Alternations the measured workload performs per 64 KiB cycle.
///
/// DERIVED FROM THE TYPED SPEC, NOT FITTED. Each `O_SYNC` `pwrite` in a cycle
/// produces two block/wake alternations (the data write, then its completion),
/// and the terminating `fdatasync` produces one: `writes_per_cycle * 2 + 1`.
/// v1's three retained wprof audits counted 4,226 / 4,228 / 4,227 on-CPU slices
/// over 128 cycles against this formula's 4,224, and a 64 MiB probe counted
/// exactly 33,792 = 33 * 1,024 with zero excess. `schedstat` `pcount` agrees
/// with wprof's slice count to within one event — measured as exactly +1 in 15
/// of 15 eligible Set B runs — while the two disagree by a variable 2.23x to
/// 4.59x (median 3.21x) on per-slice *durations*. That split is precisely why
/// the count is taken as structure and the durations only as shape: agreeing
/// on the count establishes both instruments see the same episodes, which
/// localises the disagreement to the durations alone.
const fn alternations_per_cycle(spec: &IoModelSpec) -> Option<u64> {
    match (spec.writes_per_cycle as u64).checked_mul(2) {
        Some(doubled) => doubled.checked_add(1),
        None => None,
    }
}

/// Split `total` over `slots` parts in the fixed proportions `weights`, exactly.
///
/// The returned parts sum to `total` with no residue: the floor division's
/// remainder is handed out one nanosecond at a time by descending fractional
/// part, ties broken by ascending index. This is what makes v2's dimension-(1)
/// claim a construction rather than an approximation — see
/// [`io_model_v2_slices`].
fn apportion_exact(total: u128, weights: &[u64]) -> Option<Vec<u64>> {
    ...
    for (index, weight) in weights.iter().enumerate() {
        let scaled = total.checked_mul(u128::from(*weight))?;
        let quotient = scaled / weight_sum;
        parts.push(u64::try_from(quotient).ok()?);
        remainders.push((scaled - quotient * weight_sum, index));
        assigned += quotient;
    }
    let mut residue = total.checked_sub(assigned)?;
    remainders.sort_by(|a, b| b.0.cmp(&a.0).then(a.1.cmp(&b.1)));
    for (_, index) in remainders {
        if residue == 0 {
            break;
        }
        parts[index] = parts[index].checked_add(1)?;
        residue -= 1;
    }
    (residue == 0).then_some(parts)
}

/// The v2 alternation structure for one worker: paired on-CPU and non-running
/// durations, one pair per alternation, in emission order.
///
/// **THE INVARIANT THAT DEFINES v2.** The returned on-CPU durations sum to
/// exactly `frozen_v1_totals(spec).0` and the non-running durations to exactly
/// `frozen_v1_totals(spec).1`, as integer nanoseconds. v2 therefore
/// *apportions* v1's totals and never re-estimates them, so its dimension-(1)
/// prediction is bit-identical to v1's and cannot regress. That is a
/// pre-registered, falsifiable claim, and `io_model_v2_preserves_v1_totals`
/// checks it across the whole declared domain.
///
/// Ordering within a cycle is NOT claimed. The shape table is a marginal
/// distribution, so the model reproduces the *distribution* of slice sizes and
/// not their sequence; establishing the sequence would need a per-position
/// alignment the instruments do not support. Recorded as an approximation at
/// the lowering site rather than left implicit.
fn io_model_v2_slices(spec: &IoModelSpec) -> Option<Vec<(DurationNs, DurationNs)>> {
    let (system_total, nonrunning_total) = frozen_v1_totals(spec)?;
    let cycles = spec.declared_bytes_per_worker / spec.cycle_bytes()?;
    let per_cycle = alternations_per_cycle(spec)?;
    let alternations = usize::try_from(cycles.checked_mul(per_cycle)?).ok()?;
    if alternations == 0 || per_cycle as usize != IO_MODEL_V2_ONCPU_SHAPE_NS.len() {
        return None;
    }
    // Tile the per-cycle shape across every cycle, then apportion each total
    // over the whole tiled weight vector at once. Apportioning per cycle would
    // round `cycles` times and leave a residue of up to one nanosecond per
    // cycle against the frozen total.
    let tile = |shape: &[u64; 33]| -> Vec<u64> {
        shape.iter().copied().cycle().take(alternations).collect()
    };
    let system = apportion_exact(
        u128::from(system_total.as_nanos()),
        &tile(&IO_MODEL_V2_ONCPU_SHAPE_NS),
    )?;
    let nonrunning = apportion_exact(
        u128::from(nonrunning_total.as_nanos()),
        &tile(&IO_MODEL_V2_OFFCPU_SHAPE_NS),
    )?;
    Some(
        system
            .into_iter()
            .zip(nonrunning)
            .map(|(s, n)| (DurationNs::from_nanos(s), DurationNs::from_nanos(n)))
            .collect(),
    )
}
...
fn plan_work(
    ...
    let plan = match wt {
        ...
        W::IoModelV1 { spec } => {
            ...
            let slices =
                io_model_v2_slices(spec).ok_or_else(|| LoweringError::InvalidIoProfile {
                    profile_id: profile.profile_id.clone(),
                    why: "could not apportion the frozen totals over the derived alternation \
                          structure"
                        .into(),
                })?;
            ...
            ctx.approx(
                src,
                format!(
                    "[SystemCpu NonRunning] x {alternations} then Park once \
                     [profile_id={}, shape_id={}, bytes/worker={}]",
                    profile.profile_id,
                    IO_MODEL_V2_SHAPE_ID,
                    profile.spec.declared_bytes_per_worker,
                ),
                Cause::IoMechanism,
                format!(
                    "typed fixed-volume spec={spec:?}; {cycles} cycles become {alternations} \
                     block/wake alternations at {} per cycle, DERIVED from the spec as \
                     writes_per_cycle*2+1 rather than fitted. The two totals are v1's frozen \
                     values, apportioned over those alternations and summing to them exactly, so \
                     the time totals are unchanged from v1. SLICE ORDER WITHIN A CYCLE IS NOT \
                     MODELLED: the shape table is a marginal distribution, so the distribution of \
                     slice sizes is reproduced and their sequence is not. Per-slice durations are \
                     shape-only — summed over a run, wprof's per-slice durations fall short of \
                     the kernel's sum_exec_runtime by a VARIABLE factor measured at 2.23x-4.59x \
                     (median 3.21x) over 15 runs, so no single correction factor exists and the \
                     absolute scale is discarded by the rescaling. wprof and schedstat pcount \
                     agree on the slice COUNT (+1 in 15 of 15 runs), which is what localises the \
                     disagreement to the durations. The non-running \
                     measurement may retain host-stolen time and is modelled as unrunnable rather \
            ...
            for (system, nonrunning) in slices {
                phases.push(Phase::SystemCpu(system));
                phases.push(Phase::NonRunning(nonrunning));
            }
            phases.push(Phase::Park);
            Plan {
                name_prefix: "io".into(),
                tasks: n,
                repeat: Repeat::Once,
...
#[cfg(test)]
mod io_model_v2_structure_tests {
    use super::*;

    fn spec_at(declared_bytes_per_worker: u64) -> IoModelSpec {
        IoModelSpec {
            operation: IoModelOperation::SequentialWrite,
            backing: IoModelBacking::FreshRawUnthrottledBlockDevice,
            backing_capacity_bytes: IO_MODEL_V1_BACKING_CAPACITY_BYTES,
            open_mode: IoModelOpenMode::OSync,
            write_size_bytes: 4096,
            writes_per_cycle: 16,
            flush: IoModelFlush::FdatasyncPerCycle,
    ...
    /// Every declared volume in the frozen domain, not a sampled few: the
    /// domain is only 1,023 cycles wide, so exhaustiveness is affordable and
    /// removes any question of a lucky sample.
    fn every_declared_volume() -> impl Iterator<Item = u64> {
        (IO_MODEL_V1_MIN_BYTES..=IO_MODEL_V1_MAX_BYTES).step_by(65_536)
    }

    /// v2's headline claim, as a test that fails if the claim stops holding.
    #[test]
    fn io_model_v2_preserves_v1_totals() {
        for bytes in every_declared_volume() {
            let spec = spec_at(bytes);
            let (system_total, nonrunning_total) =
                frozen_v1_totals(&spec).expect("volume is inside the frozen domain");
            let slices = io_model_v2_slices(&spec).expect("structure resolves in-domain");
            let system: u64 = slices.iter().map(|(s, _)| s.as_nanos()).sum();
            let nonrunning: u64 = slices.iter().map(|(_, n)| n.as_nanos()).sum();
            assert_eq!(
                system,
                system_total.as_nanos(),
                "system CPU total moved at {bytes} bytes; v2 must apportion v1's total, not re-estimate it"
            );
            assert_eq!(
                nonrunning,
                nonrunning_total.as_nanos(),
                "non-running total moved at {bytes} bytes"
            );
        }
    }

    /// The count is structural, so it is asserted against the spec arithmetic
    /// and against the two independently measured anchors.
    #[test]
    fn io_model_v2_alternation_count_is_derived_from_the_spec() {
        for bytes in every_declared_volume() {
            let spec = spec_at(bytes);
            let cycles = bytes / 65_536;
            let slices = io_model_v2_slices(&spec).expect("structure resolves in-domain");
            assert_eq!(slices.len() as u64, 33 * cycles, "at {bytes} bytes");
        }
        // 8 MiB: v1's three wprof audits counted 4,226/4,228/4,227 slices and
        // schedstat pcount independently read 4,227, against 33*128 = 4,224
        // plus a 2-4 slice startup preamble the model does not emit.
        assert_eq!(
            io_model_v2_slices(&spec_at(8 * 1024 * 1024)).unwrap().len(),
            4_224
        );
        // 64 MiB: a wprof probe counted exactly 33,792 slices, zero excess.
        assert_eq!(
            io_model_v2_slices(&spec_at(64 * 1024 * 1024))
                .unwrap()
                .len(),
            33_792
        );
    ...
    /// A uniform-slice model would satisfy the totals and the count while
    /// destroying the distribution — the failure mode dimension (2b) exists to
    /// catch. Guard that the emitted slices really are spread, and bimodally:
    /// measured, only 0.07-5.70% of real slices fall within +/-20% of their
    /// run's mean, so a model whose slices cluster at the mean is wrong.
    #[test]
    fn io_model_v2_emits_a_bimodal_spread_not_a_constant() {
        let slices = io_model_v2_slices(&spec_at(8 * 1024 * 1024)).unwrap();
        let system: Vec<u64> = slices.iter().map(|(s, _)| s.as_nanos()).collect();
        let mean = system.iter().sum::<u64>() as f64 / system.len() as f64;
        let near_mean = system
            .iter()
            .filter(|ns| ((**ns as f64) - mean).abs() / mean <= 0.20)
            .count();
        assert!(
            (near_mean as f64) / (system.len() as f64) < 0.20,
            "emitted slices cluster at the mean ({near_mean}/{}), which is the \
             uniform-slice failure mode v2 exists to fix",
            system.len()
        );
        let min = *system.iter().min().unwrap();
        let max = *system.iter().max().unwrap();
        assert!(max >= 3 * min, "emitted spread {min}..{max} is not bimodal");
    }

    /// Apportionment is exact for adversarial weightings, not just the frozen
    /// table: residues must never be silently dropped or double-counted.
    #[test]
    fn apportion_exact_is_exact() {
        for total in [0u128, 1, 2, 7, 1_000_000_007, u128::from(u32::MAX)] {
            for weights in [
                vec![1u64],
                vec![1, 1, 1],
                vec![1, 1_000_000],
                ...
                let parts = apportion_exact(total, &weights).expect("weights are positive");
                assert_eq!(parts.len(), weights.len());
                assert_eq!(
                    parts.iter().map(|p| u128::from(*p)).sum::<u128>(),
                    total,
        ...
        assert!(
            apportion_exact(10, &[]).is_none(),
            "empty weights must fail closed"
        );
        assert!(
            apportion_exact(10, &[0, 0]).is_none(),
            "zero weights must fail closed"
        );
    }
}
