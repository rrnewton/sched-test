...
///
/// The measured scale is NOT claimed to be correct in absolute terms: wprof's
/// per-slice durations disagree with two kernel-native instruments by ~2.7x
/// (see `experiments/io_model_v2_20260814/PREREGISTRATION.md`, Phase 0 finding
/// 3). Only their SHAPE is used, and the rescaling discards the disputed
/// scale. This is why the calibration is honest despite the open discrepancy.
...
/// with wprof's slice count to within one event while the two instruments
/// disagree ~2.7x on per-slice *durations* — which is why the count is taken
/// as structure and the durations only as shape.
            ...
            ctx.approx(
                ...
                format!(
                    "typed fixed-volume spec={spec:?}; {cycles} cycles become {alternations} \
                     ...
                     shape-only — wprof's absolute per-slice scale is contradicted ~2.7x by two \
                     kernel-native instruments and is discarded by the rescaling. The non-running \
