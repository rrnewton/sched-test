...
/// Exercise `exiting_task_workaround_enabled=true` (the manifest sets it
/// false). No task here is exiting when its cgrp_ctx is looked up, so the
/// root-cgroup fallback does not trigger -- but having the flag enabled
/// exercises the guard checks differently.
#[test]
...
/// Combine 33+ CPUs with SMT and timer reconfig to exercise dump paths with
/// large cpumasks alongside other coverage targets.
#[test]
#[ignore = "mb sim-c923d6: upstream (0f579b78/b62f1bae) removed the BPF cell allocator, update_timer map, and configuration_seq global; this test drives the deleted timer/reconfig path. Re-enable after rewrite for userspace-only cell-control."]
