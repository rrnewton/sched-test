...
#[test]
#[ignore = "sim-rmeu2 + sim-r7eou: this builds cells as cgroup cpusets and then migrates between them at runtime. Since sim-4qlh5 (PR #79) the engine ENFORCES cgroup cpusets on task placement, so the migrant is confined to cell_a's two CPUs (measured: 196447325ns of a ~400ms sim, against a floor calibrated for the unconfined world), and sim-r7eou records that a runtime cgroup migration does not yet re-narrow the cpumask, so it never gains cell_b's CPUs either. Needs a mitosis-owner decision on how a cell should be expressed, not a lowered floor — same call PR #79 made for test_mitosis_cpu_borrowing_enqueue and test_mitosis_demand_rebalancing."]
...
#[test]
#[ignore = "sim-rmeu2: this builds two 4-CPU cells as cgroup cpusets and asserts the workload spreads over both. Since sim-4qlh5 (PR #79) a cgroup cpuset is a hard constraint, so each cgroup's tasks stay inside their own half and the run uses 2 CPUs where the assertion wants 4+. A cgroup cpuset is not a mitosis cell; the modelling is what is wrong, not the engine. Needs a mitosis-owner decision, not a lowered CPU count."]
