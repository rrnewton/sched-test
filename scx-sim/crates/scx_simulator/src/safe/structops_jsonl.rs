...
fn emit_event(event: &TraceEvent, writer: &mut impl Write) -> io::Result<()> {
    ...
    match &event.kind {
        ...
        TraceKind::SetWeight { pid, weight } => emit_line(
            writer,
            ts,
            cpu,
            pid.0,
            ...
            "set_weight",
            "entry",
            &format!(r#""task_pid":{},"weight":{}"#, pid.0, weight),
            None,
        )?,
        TraceKind::Disable { pid } => emit_line(
            writer,
            ts,
            cpu,
            pid.0,
        ...
        TraceKind::TaskYield { pid, handled } => emit_line(
            writer,
            ts,
            cpu,
            pid.0,
            ...
            "yield",
            "entry",
            &format!(r#""task_pid":{},"handled":{}"#, pid.0, handled),
            None,
        )?,
