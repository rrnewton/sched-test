...
pub(crate) unsafe fn batch_worker_body<S: Scheduler>(
    ...
    // Reconstruct a SimState from the raw pointer components.
    // SAFETY: All pointers come from fields of the same SimState in
    // process_batch_concurrent. Token passing ensures exclusive access.
    // We use ManuallyDrop to avoid running destructors on the reconstructed
    // SimState — the data is still owned by the caller's SimState.
    let mut sim_state = std::mem::ManuallyDrop::new(SimState {
        sim: std::ptr::read(sp),
        tasks: std::ptr::read(tasks),
        events: std::ptr::read(events),
        cgroup_registry: std::ptr::read(cgroups),
    });
    ...
    // Write back the modified state to the raw pointers.
    std::ptr::write(sp, std::ptr::read(&sim_state.sim));
    std::ptr::write(tasks, std::ptr::read(&sim_state.tasks));
    std::ptr::write(events, std::ptr::read(&sim_state.events));
    std::ptr::write(cgroups, std::ptr::read(&sim_state.cgroup_registry));
}
...
impl<S: Scheduler> Simulator<S> {
            ...
            if interleave_enabled {
                ...
                for event in batch {
                    if let Some(err) = self.process_event(
                        event, &mut s, watchdog_timeout,
                        scenario.duration_ns, max_cgroups, monitor,
                    ) {
                        exit_kind = err;
                        break 'event_loop;
                    }
        ...
        match event.kind {
            ...
            EventKind::CgroupMigrate {
                ...
                self.handle_cgroup_migrate(
                    pid, &from_cgroup, &to_cgroup, s, monitor,
                );
            }
            ...
            EventKind::IrqStart {
                ...
                self.handle_irq_start(
                        cpu, irq_type, duration_ns, &wake_pids, s, monitor,
                    );
            }
            EventKind::IrqEnd { cpu } => {
    ...
    fn handle_timer_fired(
        ...
        if let Some(fire_at) = s.sim.pending_timer_ns.take() {
            ...
            s.events.push(fire_at, EventKind::TimerFired { cpu: timer_cpu });
        } else {
    ...
    fn handle_cgroup_migrate(
        ...
        unsafe {
            ...
            start_rbc(&mut s.sim);
            self.scheduler
                .cgroup_move(raw, from_raw, to_raw);
        ...
        loop {
            if per_cpu.len() >= 2 {
                self.process_batch_concurrent(
                    per_cpu, s, watchdog_timeout,
                    duration_ns, max_cgroups, monitor,
                );
