...
// scx_layered's real core-growth policy (`pub mod layer_core_growth`),
// compiled verbatim.
//
// The `#[path]` attribute declaring it is GENERATED by `build.rs` into OUT_DIR
// from `scxsim_build::resolve_scx_root`, because a `#[path]` written here would
// have to be a string literal — nailed to the bundled submodule and blind to
// `SCX_ROOT`. See `build.rs` for what that silently produced.
include!(concat!(env!("OUT_DIR"), "/layer_core_growth_mod.rs"));
