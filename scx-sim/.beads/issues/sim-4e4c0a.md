...
updated_at: 2026-07-23T04:33:02.551523895+00:00
---
...
Update: my duplicate test file (cgroup_bw_throttle_cycle.rs, commit 4b47780) was REMOVED (929abf8) as a concurrent duplicate of the first-landed cgroup_bw_throttle_unthrottle.rs (edba5b8). This substrate gap remains valid & OPEN: a nested cgroup with NO own cpu.max is not bounded by its ancestor's limit under the sim (bpf_cgroup_ancestor NULL stub -> parent_id=0). The surviving test file covers nested throttle by limiting the nested cgroup DIRECTLY (cgroup_nested_bw), which throttles on its own budget and thus does not exercise/expose this ancestor-propagation gap. FIX unchanged: model bpf_cgroup_ancestor().
