...
#   - rt-app installed:   pinned via $SCXSIM_RTAPP_BIN, or at ~/bin/rt-app
#                         (see: make check-deps)
...
# Same resolution order as rtapp_bin() in real_run.rs and `make check-deps`:
# $SCXSIM_RTAPP_BIN, then ~/bin/rt-app. PATH is deliberately NOT consulted:
# real_run.rs passes the resolved path into the VM (virtme-ng preserves $HOME),
# so accepting an arbitrary host PATH location here would diverge from what the
# Rust does and from what check-deps reports.
RTAPP_BIN="${SCXSIM_RTAPP_BIN:-${HOME}/bin/rt-app}"
